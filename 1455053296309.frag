...
void main(){
    ...
    float a = getNumber(u_tex0,u_tex0Resolution, u_time, 1.);
    float b = getNumber(u_tex0,u_tex0Resolution, u_time, 2.);
    float c = getNumber(u_tex0,u_tex0Resolution, u_time, 3.);
    float d = getNumber(u_tex0,u_tex0Resolution, u_time, 4.);
    
    vec2 translate = vec2(a,b);

    color += vec3(cross(st+translate,0.25));
