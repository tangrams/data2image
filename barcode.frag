...
float barCode(vec2 st, float rows, float value) {
    ...
    if (ipos.x == 0.0 ) {
        ...
        if (ipos.y == 0.0) {
            return char(fpos,value);
        } else {
            return 1.-bar(fpos,value);
        }
    }
}

vec2 getCoord(vec2 res, float col, float row) {
    return fract(vec2(col,row+.25)/res);
}
...
float getNumber(sampler2D tex, vec2 res, float col, float row) {
    ...
    float press = value.a*255.-128.;
    float s = sign(press);
    press = pow(10.,-floor(abs(press)));
...
void main(){
    ...
    vec2 pos = vec2(u_time,4.0)+vec2(.25);
    float value = floor(getNumber(u_tex0,u_tex0Resolution,pos.x,pos.y));
    vec4 tex = texture2D(u_tex0,st);
    vec2 coord = getCoord(u_tex0Resolution,pos.x,pos.y);
    vec2 zone = step(coord,st)-step(coord+vec2(.01),st);
